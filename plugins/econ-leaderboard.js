...
▰▱▰▱▰▱▰▱▰▱▰▱▰▱
々 *D I A M O N D*
│▸ Top Global ${len} *Diamond*
│▸ Kamu : *${usersLim.indexOf(m.sender) + 1}* dari *${usersLim.length}*
└──···

${sortedLim.slice(0, len).map(({ jid, diamond }, i) => `*${i + 1}.* ${participants.some(p => areJidsSameUser(jid, p.id)) ? `${conn.getName(jid)}\nwa.me/` : '@'}${jid.split`@`[0]} ▸ _*Diamond ${diamond}*_`).join`\n`}
▰▱▰▱▰▱▰▱▰▱▰▱▰▱
々 *L E V E L*
...
▰▱▰▱▰▱▰▱▰▱▰▱▰▱
`.trim()
  conn.sendFile(m.chat, lele, 'lele.jpg', text, m, {
    mentions: [...usersExp.slice(0, len), ...usersLim.slice(0, len), ...usersMoney.slice(0, len), ...usersLevel.slice(0, len)].filter(v => !participants.some(p => areJidsSameUser(v, p.id) )) 
})
